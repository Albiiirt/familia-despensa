    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-t-3xl shadow-2xl z-10 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 pb-3">
          <h2 className="font-bold text-slate-800 text-lg">{item ? 'Editar producto' : 'Añadir producto'}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-5 pb-6 space-y-4 overflow-y-auto max-h-[75vh] hide-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Cantidad actual</label>
              <input
                type="number"
                min={0}
                step="any"
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Mínimo deseado</label>
              <input
                type="number"
                min={0.01}
                step="any"
          ...
          {/* Notes */}
          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Notas (opcional)</label>
            <input
              type="text"
              value={form.notes}
              onChange={e => set('notes', e.target.value)}
              placeholder="Marca preferida, dónde comprar..."
              className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-800 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition"
            />
          </div>
